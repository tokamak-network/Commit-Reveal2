...
contract CommitReveal2OnChain is Ownable {
    ...
    mapping(uint256 round => mapping(address operator => uint256 commitOrder)) public s_commitOrders;
    mapping(uint256 round => uint256 randomNum) public s_randomNum;

    constructor(uint256 d, uint256 requestFee, uint256 commitDuration, uint256 reveaDuration) Ownable(msg.sender) {
        s_d = d;
    ...
    function commit(uint256 cv) external {
        require(s_depositAmount[msg.sender] >= s_d, DepositLessThanActivationThreshold());
        mapping(address operator => uint256 commitOrder) storage commitOrders = s_commitOrders[s_round];
    ...
    function firstReveal2(bytes32 secret, uint256[] calldata revealOrders) external {
        require(block.timestamp >= s_reveal2StartTime, Reveal2PhaseNotStarted());
        ...
        require(firstRevealOrder == s_commitOrders[s_round][msg.sender] - 1, NotTheFirstRevealer());
        require(keccak256(abi.encodePacked(secret)) == bytes32(s_cos[firstRevealOrder]));
    ...
    function reveal2(bytes32 secret) external {
        ...
        uint256 commitIndex = s_commitOrders[s_round][msg.sender] - 1;
        require(s_revealOrders[secretLength] == commitIndex, RevealNotInAscendingOrder());
        require(keccak256(abi.encodePacked(secret)) == bytes32(s_cos[commitIndex]));
        s_secrets.push(secret);
        if (secretLength + 1 == s_cvs.length) {
            s_randomNum[s_round] = uint256(keccak256(abi.encodePacked(s_secrets)));
        }
    }
    ...
    function efficientKeccak256(uint256 a) private pure returns (uint256 value) {
    ...
    function deposit() external payable {
        ...
        uint256 d = s_d;
        unchecked {
            if ((s_depositAmount[msg.sender] += msg.value) >= d && depositAmount < d) ++s_canParticipateNum;
        }
    }
    ...
    function withdraw(uint256 amount) external {
        ...
        uint256 d = s_d;
        unchecked {
            if ((s_depositAmount[msg.sender] -= amount) < s_d && depositAmount >= d) --s_canParticipateNum;
        }

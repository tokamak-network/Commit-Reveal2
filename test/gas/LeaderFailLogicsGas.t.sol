...
contract LeaderFailLogicsGas is BaseTest, CommitReveal2Helper {
    ...
    function test_failToRequestSubmitCvOrSubmitMerkleRootGas() public {
        ...
        finalOutput = vm.serializeString(
            "any string",
            "description",
            "Leader fail: Path 1->4. Varies by operators count. Period parameters (off-chain, decision windows) determine elapsed time; calldata is constant."
        );
    ...
    function test_failToSubmitMerkleRootAfterDisputeGas() public {
        ...
        finalOutput = vm.serializeString(
            "any string",
            "description",
            "Leader fail: Path 1->2->3->7. Varies by operators count and number of Cvs submitted (all in this test). Gas depends on bitmap size and storage touched while penalizing after on-chain period."
        );
    ...
    function test_failToRequestSorGenerateRandomNumberGas() public {
        ...
        finalOutput = vm.serializeString(
            "any string",
            "description",
            "Leader fail: Path 1->5->14. Varies by operators count. After all Cvs on-chain and MerkleRoot submitted, waiting windows scale with operator count; calldata fixed."
        );

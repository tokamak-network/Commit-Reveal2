...
contract CommitReveal2Test is NormalPathBaseTest, Utils {
    ...
    function setUp() public override {
        ...
        commitReveal2 = new CommitReveal2OnChain(s_activationThreshold, s_requestFee, s_commitDuration, s_reveaDuration);
        vm.stopPrank();
    ...
    function test_OnChainCommitReveal2() public {
        ...
        console2.log("---------------------------");
        uint256 i = 0;
        for (uint256 numOfOperators = 2; numOfOperators <= s_maxOperators; numOfOperators++) {
            ...
            for (uint256 round = i++ * requestTestNum; round < requestTestNum * i; round++) {
                // ** Request Random Number
                commitReveal2.requestRandomNumber{value: s_requestFee}();
                if (round % requestTestNum != 0) {
                    gasUsedRequest[(round % requestTestNum) - 1] = vm.lastCallGas().gasTotalUsed;
                }
                ...
                commitReveal2.firstReveal2(secretValues[revealOrders[0]], revealOrders);
                if (round % requestTestNum != 0) {
                    if (round % requestTestNum != 0) {
                        sum += vm.lastCallGas().gasTotalUsed;
                    }
                ...
                // ** Assert
                bytes32[] memory secretsInRevealOrder = new bytes32[](numOfOperators);
                for (uint256 j; j < numOfOperators; j++) {
                    secretsInRevealOrder[j] = secretValues[revealOrders[j]];
                }
                uint256 calculatedRandomNum = uint256(keccak256(abi.encodePacked(secretsInRevealOrder)));

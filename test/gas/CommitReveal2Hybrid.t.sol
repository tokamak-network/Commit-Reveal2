...
import {NormalPathBaseTest} from "./../shared/NormalPathBaseTest.t.sol";
import {Utils} from "./../shared/Utils.t.sol";
import {console2} from "forge-std/Test.sol";
import {Sort} from "./../shared/Sort.sol";

contract CommitReveal2Test is NormalPathBaseTest, Utils {
    CommitReveal2Hybrid public s_commitReveal2Hybrid;

    function setUp() public override {
        NormalPathBaseTest.setUp(); // Start Prank
        // *** Deploy
        s_commitReveal2Hybrid = new CommitReveal2Hybrid(s_activationThreshold, s_requestFee, name, version);
    ...
    function test_HybridCommitReveal2() public {
        ...
        uint256 i;
        for (uint256 numOfOperators = 2; numOfOperators <= 10; numOfOperators++) {
            console2.log("Number of Operators: ", numOfOperators);
            uint256[] memory gasUsedRequest = new uint256[](s_requestTestNum - 1);
            uint256[] memory gasUsedSubmitMerkleRoot = new uint256[](s_requestTestNum - 1);
            uint256[] memory gasUsedGenerateRandomNumber = new uint256[](s_requestTestNum - 1);

            // *** Request Random Number
            for (uint256 round = i++ * s_requestTestNum; round < s_requestTestNum * i; round++) {
                s_commitReveal2Hybrid.requestRandomNumber{value: s_requestFee}();
                if (round % s_requestTestNum != 0) {
                    gasUsedRequest[(round % s_requestTestNum) - 1] = vm.lastCallGas().gasTotalUsed;
                }
                ...
                for (uint256 j; j < numOfOperators; j++) {
                    secretValues[j] = keccak256(abi.encodePacked(numOfOperators, round, j, block.timestamp));
                ...
                vm.stopPrank();
                if (round % s_requestTestNum != 0) {
                    gasUsedSubmitMerkleRoot[(round % s_requestTestNum) - 1] = vm.lastCallGas().gasTotalUsed;
                }
                ...
                uint256[] memory revealOrdersIndex = new uint256[](numOfOperators);
                for (uint256 j; j < numOfOperators; j++) {
                    revealOrders[j] = uint256(rv) > uint256(cvs[j])
                        ? uint256(keccak256(abi.encodePacked(uint256(rv) - uint256(cvs[j]))))
                        : uint256(keccak256(abi.encodePacked(uint256(cvs[j]) - uint256(rv))));
                ...
                for (uint256 j; j < numOfOperators; j++) {
                    bytes32 typedDataHash = getHashTypedDataV4(address(s_commitReveal2Hybrid), round + 1, cvs[j]);
                    (uint8 v, bytes32 r, bytes32 s) = vm.sign(s_anvilDefaultPrivateKeys[j], typedDataHash);
                    vs[j] = v;
                    rs[j] = r;
                    ss[j] = s;
                }
                // * broadcast
                s_commitReveal2Hybrid.generateRandomNumber(secretValues, vs, rs, ss);
                if (round % s_requestTestNum != 0) {
                    gasUsedGenerateRandomNumber[(round % s_requestTestNum) - 1] = vm.lastCallGas().gasTotalUsed;
                }
                assertEq(
                    s_commitReveal2Hybrid.s_randomNum(round + 1), uint256(keccak256(abi.encodePacked(secretValues)))
                );
            }

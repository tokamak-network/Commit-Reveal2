...
contract NodesFailLogicsGas is BaseTest, CommitReveal2Helper {
    ...
    function test_failToSubmitCvGas() public {
        ...
        finalOutput = vm.serializeString(
            "failToSubmitCvGas",
            "description",
            "Nodes fail: Path 1->2->3->6. Varies by operators count (XX), number of requested Cv (YY), and number who did not submit (ZZ). Larger YY/ZZ increase bitmaps/loops and deactivation/reactivation work, affecting gas."
        );
    ...
    function test_failToSubmitCoGas() public {
        ...
        finalOutput = vm.serializeString(
            "failToSubmitCoGas",
            "description",
            "Nodes fail: Path 1->5->8->10. Varies by operators count (XX), requested Co (YY), and number who did not submit (ZZ). Parameter sizes (indices arrays, bitmaps) and storage updates scale with YY/ZZ, impacting gas."
        );
    ...
    function test_failToSubmitSGas() public {
        ...
        finalOutput = vm.serializeString(
            "failToSubmitSGas",
            "description",
            "Nodes fail: Path 1->5->12->13->15. Varies by operators count (XX), S-requested length (= requested_YY via k=n-YY), and number who did not submit (ZZ). As YY/ZZ change, calldata for secrets and touched storage scale, changing gas."
        );
